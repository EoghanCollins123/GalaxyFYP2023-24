...
for n=1:Iterations                                              % Loop to perform the operation of the simulation equal to a pre-defined number of timesteps
    ...
    zeta=max(density);              % Define the maximum density value in the system
    if zeta>=6                      % Condition is max value is greater than 5 for some reason
        zeta=6;                     % If max density >5 set as 6
    end
    density=density+(6-zeta);       % OFC method of driving system
